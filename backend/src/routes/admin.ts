...
import { getUsers, getUserDetails, approveUser, rejectUser, deactivateUser, markUserCompleted, resendUserCredentials, deleteUserCompletely, createInstructor, getAdminStats, getAdminActivities } from '../controllers/adminController';
...
router.get('/payments/:studentId/dossier', getStudentPaymentDossier);
router.put('/payments/slips/:slipId/verify', verifyPaymentSlip);
router.put('/payments/slips/:slipId/reject', rejectPaymentSlip);
router.post('/payments/:studentId/record-manual', recordManualPayment);
...
router.put('/users/:id/complete', markUserCompleted);
router.put('/users/:id/resend-credentials', resendUserCredentials);
